...
Value smsgdumpairdropentries(const json_spirit::Array& params, bool fHelp) {
    if (fHelp || params.size() > 1)
        throw runtime_error(
                "smsgdumpairdropentries \"filename\"\n"
                "Dumps all airdrop entries into file"
                "\nArguments:\n"
                "1. \"path\"  (string, required) The full path to the dump.\n");
        ...
        while (dbInbox.NextSmesg(it, sPrefix, chKey, smsgStored)) {
            ...
            if (SecureMsgDecrypt(false, smsgStored.sAddrTo, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0) {
                ...
                if (json_spirit::read_string(message, jsonMessage)) {
                    ...
                    LogPrintf("JSON parse: %s", obj["ethAddress"].get_str());
                    if (!obj["ethAddress"].get_str().empty())
                    {
                        Object objM;
                        objM.push_back(Pair("received", smsgStored.timeReceived));
                        objM.push_back(Pair("from", msg.sFromAddress));
                        objM.push_back(Pair("ethAddress", obj["ethAddress"].get_str()));
                        allSignUpsArray.push_back(objM);
    ...
    write_string(json_spirit::Value(allSignUps), false).c_str();

    allSignUps.push_back(Pair("allSignUps", allSignUpsArray));
    fprintf(fp, "%s", write_string(json_spirit::Value(allSignUps), false).c_str());
    fclose(fp);
